...
/*
This file should be included rather than compiled
*/

static inline uint perm_pos(uint n, uint i, uint *p) {
	for (uint j = 0; j < n; j++)
		if (p[j] == i)
			return j;
	return 0;
...
static void init_permute(uint n, uint *p, uint *v) {
	for (uint i = 0; i < n; i++) {
		p[i] = i;
		v[i] = 0;
	}
...
static int permute(uint n, uint *p, uint *v) {
	uint i = n;
	while (i) {
		--i;
		uint pi = perm_pos(n, i, p);
		// Determine if mobile
		if (pi == 0 && !v[pi])
			continue;
		if (pi == n - 1 && v[pi])
